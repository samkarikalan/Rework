...
/* ── New clean entry: show page immediately, then load data ── */
async function openSessionPage(sessionId) {
viewerStopPoll();

// Step 1 — show the page immediately, no waiting
_vShowPage();

// Step 2 — show loading state
var container = document.getElementById(‘viewerResults’);
if (container) {
container.innerHTML = ‘<div style="padding:40px;text-align:center;color:var(--muted)"><div class="help-spinner"></div></div>’;
}

// Step 3 — fetch session data
try {
var rows = await sbGet(‘sessions’,
‘id=eq.’ + sessionId + ‘&select=id,rounds_data,started_by,created_at,updated_at,status’
);

```
if (!rows || !rows.length) {
  if (container) container.innerHTML = '<div style="padding:40px;text-align:center;color:var(--muted)">Session not found.</div>';
  return;
}
...
if (container) container.innerHTML = ‘<div style="padding:40px;text-align:center;color:var(--muted)">Could not load session. Check connection.</div>’;
}
}
