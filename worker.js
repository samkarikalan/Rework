...
function selectRestingAndPlaying(state) {
  ...
  if (fixedPairs.length && numResting >= 2) {
    ...
    // Use restQueue order for playing — client keeps it rotation-aware
    playing = [...restQueue]
      .filter(p => activeplayers.includes(p) && !resting.includes(p))
      .slice(0, playersPerRound);
    // Add any active players not yet in restQueue (edge case: new joiners)
    const inPlaying = new Set(playing);
    for (const p of activeplayers) {
      if (!inPlaying.has(p) && !resting.includes(p) && playing.length < playersPerRound) {
        playing.push(p);
        inPlaying.add(p);
      }
    }
...
function getNextFixedPairGames(state, fixedPairs, numCourts) {
  ...
  if (!state.fixedPairGameQueue || !state.fixedPairGameQueue.length || state.fixedPairGameQueueHash !== hash) {
    ...
    state.fixedPairGameQueue = shuffle(
...
function randomRound(state) {
  ...
  if (!freePlayers.length && fixedPairs.length >= numCourts * 2) {
...
function validateRound(rnd, state) {
  ...
  // Court count mismatch is allowed — worker fills what it can, rest stay empty
  const seen = new Set();
...
// ── Singles matchup picker ──
// Scores every 1v1 pair by opponentMap (times faced each other).
// Picks the freshest matchup — mirrors how doubles uses bestGame().
// Pool must already be in rotation order (restQueue-ordered by client).
function singlesMatchup(pool, opponentMap) {
  if (pool.length < 2) return null;
  let bestScore = Infinity, bestP1 = pool[0], bestP2 = pool[1];
  for (let i = 0; i < pool.length; i++) {
    for (let j = i + 1; j < pool.length; j++) {
      const times = ((opponentMap[pool[i]] || {})[pool[j]] || 0);
      if (times < bestScore) {
        bestScore = times;
        bestP1 = pool[i]; bestP2 = pool[j];
        if (times === 0) { i = pool.length; break; } // can't do better
      }
    }
  }
  return { pair1: [bestP1], pair2: [bestP2] };
}
...
function typedRound(state) {
  ...
  // ── 3. Gender pools — preserve playing order when rest counts are equal ──
  // sortRested only re-orders when players have genuinely different rest counts.
  // When all counts are equal (e.g. no one has rested yet), preserve the
  // incoming `playing` order, which is already shuffled by the client.
  const sortRested = pool => {
    const arr = [...pool];
    const allEqual = arr.every(p => (restCount[p] || 0) === (restCount[arr[0]] || 0));
    if (allEqual) return arr; // preserve shuffle — stable no-op sort would reset to arrival order
    return arr.sort((a, b) => (restCount[b] || 0) - (restCount[a] || 0));
  };
  ...
  function rebuildPools() {
    // After a gender swap we DO want rest-count order to pick the right people
    const sortForSwap = pool =>
      [...pool].sort((a, b) => (restCount[b] || 0) - (restCount[a] || 0));
    allMenPlaying   = sortForSwap(playing.filter(p => getGender(p, allPlayers) === 'Male'));
    allWomenPlaying = sortForSwap(playing.filter(p => getGender(p, allPlayers) === 'Female'));
  }
  ...
  // Build court order: typed courts first, free courts after
  const courtOrder = [];
  for (let c = 0; c < numCourts; c++) {
    const fmt  = courtFormats[c] || 'doubles';
    const type = effectiveType(courtTypes[c] || 'free', fmt);
    if (type !== 'free' && type !== 'singles-free') courtOrder.push(c);
  }
  for (let c = 0; c < numCourts; c++) {
    const fmt  = courtFormats[c] || 'doubles';
    const type = effectiveType(courtTypes[c] || 'free', fmt);
  ...
  for (const c of courtOrder) {
    ...
    if (fmt === 'singles') {
      ...
      // Use scored matchup (via opponentMap) to avoid repeat opponents — same logic as doubles.
      const pool = type === 'singles-men'   ? availMen()   :
                   type === 'singles-women' ? availWomen() :
                   availAll(); // singles-free
      const sm = singlesMatchup(pool, opponentMap);
      if (sm) {
        game = { court: c + 1, pair1: sm.pair1, pair2: sm.pair2, isSingles: true };
        consume([...sm.pair1, ...sm.pair2]);
      }
    ...
    if (!game) {
      ...
      if (fmt === 'singles') {
        const sm = singlesMatchup(pool, opponentMap);
        if (sm) {
          game = { court: c + 1, pair1: sm.pair1, pair2: sm.pair2, isSingles: true, isFallback: true };
          consume([...sm.pair1, ...sm.pair2]);
        }
      } else {
        const pairs = pickPairs(pool, 2);
...
async function handleGenerateRound(request, env) {
  ...
  // Use typedRound if any court has a non-free type
  const hasTypedCourts = (state.courtTypes || []).some(t => t && t !== 'free') ||
                         (state.courtFormats || []).some(f => f === 'singles');

  const useComp = state.playMode === 'competitive';
  if (useComp && state.lastMode !== 'competitive') resetForCompetitive(state);
  // Increment roundIndex once here, not inside each generator (retry loop would triple-count)
  state.roundIndex = (state.roundIndex || 0) + 1;

  let result = null;
  for (let attempt = 0; attempt < 3; attempt++) {
    result = hasTypedCourts ? typedRound(state) : (useComp ? competitiveRound(state) : randomRound(state));
