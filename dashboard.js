...
container.innerHTML = ` <div class="dash-empty"> <div class="dash-empty-icon">👤</div> <p>Set up your profile first.</p> <p style="font-size:0.78rem;color:var(--text-dim);margin-top:4px">Tap the profile icon to select your name.</p> </div>`;
return;
}
if (!isViewer && (!club || !club.id)) {
container.innerHTML = ` <div class="dash-empty"> <div class="dash-empty-icon">🏟️</div> <p>No club selected.</p> <p style="font-size:0.78rem;color:var(--text-dim);margin-top:4px">Go to Club tab to join a club.</p> </div>`;
return;
}
...
container.innerHTML = ` <div class="dash-empty"> <div class="dash-empty-icon">📡</div> <p>Could not load sessions.</p> <p style="font-size:0.78rem;color:var(--text-dim);margin-top:4px">Check your connection.</p> <button class="help-retry-btn" onclick="renderDashboard()" style="margin-top:12px">↺ Retry</button> </div>`;
}
}
...
top.innerHTML = `<div class="dash-card-club">${clubName || 'Club'}</div> ${isLive ?`<div class="dash-live-badge"><div class="dash-live-dot-sm"></div>LIVE</div>`:`<div class="dash-past-badge">${dateLabel}</div>`} `;
card.appendChild(top);

// Meta row
const meta = document.createElement(‘div’);
meta.className = ‘dash-card-meta’;
meta.innerHTML = `<span>👥 ${players.length} players</span> ${totalRounds ?`<span>🔄 ${totalRounds} rounds</span>`: ''} ${starter ?`<span>▶ ${starter}</span>`: ''}`;
...
dashboardStopPoll();
if (typeof openSessionPage === ‘function’) openSessionPage(sessionId);
}
