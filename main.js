...
var _umsSwitching = false;      // true when user clicked “Switch club ›”

function openModeSwitcher() {
const existing = document.getElementById(‘modeSheetOverlay’);
if (existing) { _umsSwitching = false; existing.remove(); return; }
...
clubSection = `<div class="ums-connected-club"> <div class="ums-club-row"> <span class="ums-club-icon">🏢</span> <div class="ums-club-info"> <div class="ums-club-name">${club.name} ${roleTag}</div> <div class="ums-club-sub">Connected club</div> </div> </div> ${m === 'vault' && !isAdmin ?`
...
`:`<div id="umsFeedback" class="ums-feedback"></div>`} <button class="ums-switch-link" onclick="_umsShowLogin()">Switch club ›</button> </div>`;
} else {
...
const modeTabs = modes.map(mo => ` <button class="ums-mode-tab ${_uSheetSelectedMode === mo.key ? 'active' : ''}" onclick="_umsSelectMode('${mo.key}')"> <span class="ums-tab-icon">${mo.icon}</span> <span class="ums-tab-label">${mo.label}</span> </button>`).join(’’);
...
overlay.innerHTML = ` <div class="ums-sheet" id="umsSheet"> <div class="mode-sheet-handle"></div> <div class="ums-mode-tabs">${modeTabs}</div> <div class="ums-divider"></div> <div class="ums-club-section" id="umsClubSection">${clubSection}</div> <button class="ums-enter-btn" id="umsEnterBtn" onclick="_umsEnter()">${enterLabel} →</button> </div>`;
...
return ` <div class="ums-section-label">Club login</div> <select id="umsClubSelect" class="ums-input"> <option value="">— Loading clubs… —</option> </select> <input type="password" id="umsPassword" class="ums-input" placeholder="${isVault ? 'Admin password' : 'Club password'}" onkeydown="if(event.key==='Enter')_umsEnter()"> <div id="umsFeedback" class="ums-feedback"></div> <div class="ums-create-link-row"> New club? <button class="ums-switch-link" onclick="_umsShowCreate()">Create one ›</button> </div>`;
}
...
const section = document.getElementById(‘umsClubSection’);
if (!section) return;
section.innerHTML = ` <div class="ums-section-label">Create a new club</div> <input type="text"     id="umsCreateName"    class="ums-input" placeholder="Club name"> <input type="email"    id="umsCreateEmail"   class="ums-input" placeholder="Your email (OTP verification)"> <input type="password" id="umsCreateUserPw"  class="ums-input" placeholder="User password"> <input type="password" id="umsCreateAdminPw" class="ums-input" placeholder="Admin password"> <div id="umsFeedback" class="ums-feedback"></div> <div class="ums-create-link-row"> <button class="ums-switch-link" onclick="_umsShowLogin()">‹ Back to join</button> </div>`;
...
// Already connected to club (and not switching)
if (hasClub && !_umsSwitching) {
if (m === ‘vault’ && !isAdmin) {
// Use RPC to verify admin password (avoids RLS blocking admin_password column)
...
method: ‘POST’,
headers: SB_HEADERS,
body: JSON.stringify({ p_club_id: club.id, p_password: pw })
});
const ok = await res.json();
if (!ok) { _setUmsFb(‘Wrong admin password.’, false); return; }
...
const section = document.getElementById(‘umsClubSection’);
if (section) section.innerHTML = ` <div class="ums-section-label">Verify OTP</div> <div class="ums-otp-hint">OTP sent to <strong>${maskedEmail}</strong></div> <input type="text" id="umsOtp" class="ums-input" placeholder="8-digit OTP" maxlength="8" onkeydown="if(event.key==='Enter')_umsVerifyOtp()"> <div id="umsFeedback" class="ums-feedback"></div>`;
