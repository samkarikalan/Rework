...
```
// Fetch live sessions from sessions table (source of truth)
const sessions = await sbGet('sessions',
  `club_id=eq.${club.id}&status=eq.live&select=id,rounds_data,created_at&order=created_at.asc`
);

if (!sessions || !sessions.length) {
  container.innerHTML = '<p class="player-mgmt-empty">No active sessions right now.</p>';
  return;
}

// Extract unique player names from rounds_data
const allPlayers = [];
sessions.forEach(function(sess) {
  const seen = new Set();
  (sess.rounds_data || []).forEach(function(round) {
    (round.games || []).forEach(function(game) {
      [...(game.pair1 || []), ...(game.pair2 || [])].forEach(function(name) {
        if (!seen.has(name)) {
          seen.add(name);
          allPlayers.push({ name: name, startedAt: sess.created_at });
        }
      });
    });
  });
...
if (!allPlayers.length) {
  container.innerHTML = '<p class="player-mgmt-empty">No players currently playing.</p>';
  return;
}
...
if (admin) {
  ...
  releaseAllBtn.textContent = '🔓 Release All (' + allPlayers.length + ')';
...
allPlayers.forEach(function(p) {
  ...
  const started = p.startedAt
    ? new Date(p.startedAt).toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'})
    : '—';
  ...
  timeSpan.style.cssText = 'font-size:0.75rem;color:var(--muted);margin-right:8px;flex-shrink:0';
...
setFb(‘Saving…’, true);
try {
// 1. Update player row (using id — no schema change needed)
await sbPatch(‘players’, `id=eq.${playerId}`, {
name:        name,
gender,
club_ratings: { [getMyClub()?.id || ‘default’]: Math.round(rating * 10) / 10 },
