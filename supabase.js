...
async function sbGet(table, query = '') {
  const res = await fetch(WORKER_URL + '/db/get', {
    method:  'POST',
...
async function sbPost(table, data, prefer = 'return=representation') {
  const res = await fetch(WORKER_URL + '/db/post', {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify({ table, data, prefer })
  });
  if (!res.ok) {
...
async function sbPatch(table, query, data) {
  const res = await fetch(WORKER_URL + '/db/patch', {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify({ table, query, data })
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
...
async function sbDelete(table, query) {
  const res = await fetch(WORKER_URL + '/db/delete', {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify({ table, query })
  });
...
async function sbUpsert(table, data, onConflict) {
  const res = await fetch(WORKER_URL + '/db/upsert', {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
...
async function dbSyncRoundsData() {
  try {
    ...
    await sbPatch('sessions', `id=eq.${sessionDbId}`, { rounds_data: roundsData, updated_at: localISOString() });
...
async function dbCompleteSession(shuttleData = null) {
  try {
    ...
    const patch = { status: 'completed', players, updated_at: localISOString() };
    ...
    const all = await sbGet('sessions', `club_id=eq.${club.id}&status=eq.completed&order=updated_at.desc&select=id`);
    if (all && all.length > 3) {
      const toDelete = all.slice(3).map(s => s.id);
      for (const id of toDelete) await sbDelete('sessions', `id=eq.${id}`).catch(() => {});
    }
    setMySessionId(null);
...
async function dbCleanupStaleSessions() {
  try {
    const club = getMyClub();
    if (!club.id) return;
    const rows = await sbGet('sessions', `club_id=eq.${club.id}&status=eq.live&select=id,created_at`);
    if (!rows || !rows.length) return;
    const thirtyMinsAgo = Date.now() - (30 * 60 * 1000);
    for (const sess of rows) {
      const age = new Date(sess.updated_at || sess.created_at).getTime();
      if (age < thirtyMinsAgo) {
        await sbPatch('sessions', `id=eq.${sess.id}`, { status: 'completed', updated_at: localISOString() }).catch(() => {});
...
async function dbForceCompleteSession(sessionId) {
  try {
    await sbPatch('sessions', `id=eq.${sessionId}`, { status: 'completed', updated_at: localISOString() });
    const club = getMyClub();
    if (club.id) {
      await sbPatch('memberships', `club_id=eq.${club.id}&is_playing=eq.true`, { is_playing: false }).catch(() => {});
      const all = await sbGet('sessions', `club_id=eq.${club.id}&status=eq.completed&order=updated_at.desc&select=id`);
      if (all && all.length > 3) {
        for (const id of all.slice(3).map(s => s.id)) await sbDelete('sessions', `id=eq.${id}`).catch(() => {});
      }
    }
...
async function dbGetLiveSessions() {
  try {
    ...
    if (isViewer) {
      ...
      return await sbGet('sessions', `club_id=in.(${clubIds.join(',')})&status=eq.live&order=created_at.asc&select=id,rounds_data,started_by,updated_at,club_id`) || [];
    } else {
      const club = getMyClub();
      if (!club.id) return [];
      return await sbGet('sessions', `club_id=eq.${club.id}&status=eq.live&order=created_at.asc&select=id,rounds_data,started_by,updated_at`) || [];
    }
...
async function dbGetPastSessions() {
  try {
    ...
    if (isViewer) {
      ...
      const club = getMyClub();
      if (!club.id) return [];
      return await sbGet('sessions', `club_id=eq.${club.id}&status=eq.completed&order=updated_at.desc&limit=3&select=id,date,started_by,players,rounds_data,updated_at,shuttle_data`) || [];
    }
  } catch(e) { return []; }
}
...
function startSessionHeartbeat() {
  ...
  _heartbeatTimer = setInterval(async () => {
    ...
    try { await sbPatch('sessions', `id=eq.${sessionId}`, { updated_at: localISOString() }); }
    catch(e) {}
  }, 2 * 60 * 1000);
}
